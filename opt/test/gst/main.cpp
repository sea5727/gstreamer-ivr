...
print_stats (GstElement * rtpbin)
{
  ...
  for (i = 0; i < arr->n_values; i++) {
    ...
    val = (GValue *)g_value_array_get_nth (arr, i);
...
GstElement * parent_pipeline;
  ...
  GstElement * wavparse;
  GstElement * wavenc;
void proc_set_config_1()
{
  g_print("proc_set_config_1 start\n");
  const char * out_file = "/home/jdin/out.wav";


  filesink = gst_element_factory_make("filesink", "filesink");
  g_assert (filesink);

  g_object_set (filesink, "location", out_file, NULL);

  gst_bin_add (GST_BIN (parent_pipeline), filesink);

  g_print("proc_set_config_1 end\n");
  
}
void proc_set_config_2()
{
  g_print("proc_set_config_2 start\n");
  ...
  gst_bin_add (GST_BIN (parent_pipeline), wavparse);
  gst_bin_add (GST_BIN (parent_pipeline), wavenc);

  g_print("proc_set_config_2 end\n");
  
}

void proc_set_config_3()
{
  g_print("proc_set_config_3 start\n");
  const char * in_file = "/home/jdin/BCN3009999990.wav";
  ...
  g_object_set (filesrc, "location", in_file, NULL);


  gst_bin_add (GST_BIN (parent_pipeline), filesrc);




  g_print("proc_set_config_3 end\n");
...
main (int argc, char *argv[])
{
  ...
  /* the pipeline to hold everything */
  parent_pipeline = gst_pipeline_new (NULL);
  g_assert (parent_pipeline);
  ...
  if (!gst_element_link_many (fakesrc, wavparse, wavenc, filesink, NULL))
  {
    g_error ("Failed to link gst_element_link_many to parent_pipeline");
  }

  gst_element_set_state (parent_pipeline, GST_STATE_PLAYING);

  std::thread th([]{
    g_print("start thread\n");
    gst_element_link_many(filesrc, wavparse, NULL);
    std::this_thread::sleep_for(std::chrono::seconds(5));
    gst_element_set_state (parent_pipeline, GST_STATE_READY);
    gst_element_set_state (parent_pipeline, GST_STATE_PLAYING);
